...
if __name__ == "__main__":
    ...
    predictions = predictions + abs(np.min(predictions))
    predictions = predictions / np.max(predictions) * 255

    image_array = predictions.reshape((50, 50))

    plt.imshow(image_array, cmap='gray', vmin=0, vmax=255)
    plt.title('50x50 Grayscale Image')
